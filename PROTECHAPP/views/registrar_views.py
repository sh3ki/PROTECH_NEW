...
@login_required
@user_passes_test(is_registrar)
def registrar_calendar(request):
    """View for calendar"""
    return render(request, 'registrar/calendar.html')


@login_required
@user_passes_test(is_registrar)
def get_registrar_calendar_events(request):
    """API endpoint to get calendar events for registrar"""
    from ..models import CalendarEvent
    from django.db.models import Q
    
    # Get events and announcements targeted to registrar or all staff
    events = CalendarEvent.objects.filter(
        Q(event_type='EVENT') |  # All events
        Q(event_type='ANNOUNCEMENT', target_role__in=['REGISTRAR', 'ADMIN'])  # Announcements for registrar
    )
...
@user_passes_test(is_registrar)
@require_GET
def get_latest_registrar_attendance(request):
    """
    API endpoint for real-time attendance updates for registrar
    ...
    try:
        ...
        import traceback
        print(f"Error in get_latest_registrar_attendance: {e}")
