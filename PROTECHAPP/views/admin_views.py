...
@user_passes_test(is_admin)
@require_GET
def get_latest_attendance_records(request):
    """
    ...
    try:
        ...
        import traceback
        print(f"Error in get_latest_attendance_records: {e}")
        print(traceback.format_exc())
        return JsonResponse({
            'status': 'error',
            'message': str(e)
...
@user_passes_test(is_admin)
@require_http_methods(["POST"])
def save_holiday_mode(request):
    """Toggle holiday mode on/off to disable face recognition attendance."""
    from PROTECHAPP.models import SystemSettings

    enabled = request.POST.get('holiday_mode') == 'on'

    try:
        settings_obj, _ = SystemSettings.objects.get_or_create(pk=1)
        settings_obj.holiday_mode = enabled
        settings_obj.save()

        status_label = 'enabled' if enabled else 'disabled'
        messages.success(request, f'Holiday Mode has been {status_label}. Face recognition attendance is now {"OFF" if enabled else "ON"}.')
    except Exception as e:
        messages.error(request, f'Error saving holiday mode setting: {str(e)}')
